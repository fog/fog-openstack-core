...
module Fog
  module OpenStackCore
    class IdentityV2 < Fog::Service
      ...
      #
      # <wchrisj>	 Am trying to hit this URL, and the docs for v2 say it should
      #             work: http://devstack.local:5000/v2.0/users/{userid}/roles
      # <wchrisj>	 when I manually hit the url, I get a 404 - would I get that
      #             if there are no roles associated with the user in question?
      #             stevemar:
      # <@dolphm>	 stevemar: wchrisj: i don't think it's a supported call, as
      ...
      # <@dolphm>	 wchrisj: it's an ambiguously specified API call that we
      #             chose to never implement so as to avoid flip-flopping
      #             between the two perceivable interpretations of the spec;
      #             instead we have GET /v3/role_assignments
      # <@dolphm>	 wchrisj: which is much more powerful and avoids any confusing
      #             semantics around the call
      # <wchrisj>	 so why do the v2 docs say it exists?
      # <@dolphm>	 wchrisj: because it *may* be implemented by an alternative
      #             implementation of the API, but keystone chooses not to
      # <@dolphm>	 wchrisj: if you have authz on the rackspace public cloud, i
      #             think you'll get something back -- but you'd likely file a
      #             bug report because it's not the results you'd expect :)
      # <@dolphm>	 wchrisj: the identity service is one of the few APIs with
      #             more than one complete implementation in production floating
      #             around
      # <@dolphm>	 wchrisj: keystone just happens to be the one supported by
      #             openstack directly
      # <@dolphm>	 wchrisj: and if you look at the diablo release of keystone
      #             vs the essex release of keystone -- those were actually two
      #             completely different implementations from the ground up
      # <ayoung>	 wchrisj, we've stabilized somewhat from that point
      # <@dolphm>	 wchrisj: ++ i'd like it to be removed from openstack's api
      #             site since we don't support it directly
      ...
      class Real
        ...
        def admin_request(params)
          ...
          @admin_connection ||= admin_connection(
            :keystone, @options[:openstack_region].to_sym
          )
        ...
        def admin_connection(service_name, region)
          ...
          # get the admin url
          url = @identity_session.service_catalog.get_endpoint(
            service_name, region, :admin
          )
